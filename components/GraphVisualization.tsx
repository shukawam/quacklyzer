...
const GraphVisualization: React.FC<GraphVisualizationProps> = ({
  ...
  const { graphDefinition, nodeDataMap } = useMemo(() => {
    ...
    if (!rawData || !rawData.services)
      return {
        graphDefinition: 'graph TD\n    A[Upload a file to see the graph]',
        nodeDataMap: new Map()
      }

    let definition = 'graph TD\n'
    definition +=
      '    classDef service fill:#e0f2fe,stroke:#0ea5e9,stroke-width:2px,color:#0c4a6e\n'
    definition +=
      '    classDef route fill:#dcfce7,stroke:#22c55e,stroke-width:1px,color:#15803d\n'
    definition +=
      '    classDef plugin fill:#fefce8,stroke:#eab308,stroke-width:1px,color:#ca8a04\n'
    ...
    ;(rawData.services || []).forEach((s: any) => {
      ...
      ;(s.plugins || []).forEach((p: any) => {
        ...
        dataMap.set(pluginNodeId, {
          title: `Plugin: ${p.name} on ${s.name}`,
          data: p
        })
      })
